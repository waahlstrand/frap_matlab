...
files_estimates = dir([file_paths{idx_experiment}(1:end-4) '_est_d_rc_without_prebleach*']);
number_of_estimates = numel(files_estimates);
if number_of_estimates >= 10
    disp(['Already ' num2str(number_of_estimates) ' estimates, quitting...']);
    return
end
...
% warning('Use all for estimation of MF...')
...
file_path_output = [file_paths{idx_experiment}(1:end-4) '_est_d_rc_with_prebleach_' num2str(random_seed) '.mat'];
save(file_path_output, 'param_hat', 'ss');

% warning('Use only first 200 for estimation of D...')
% number_of_images = min(200, number_of_images);
[param_hat, ss] = estimate_d( ...
    [], ...
    data, ...
    param_bleach, ...
...
file_path_output = [file_paths{idx_experiment}(1:end-4) '_est_d_rc_without_prebleach_' num2str(random_seed) '.mat'];
save(file_path_output, 'param_hat', 'ss');
