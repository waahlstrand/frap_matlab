...
for current_frame = 1:exp_sim_param.number_of_prebleach_frames
    rc_prebleach(current_frame) = mean(mean( C_prebleach(:, :, current_frame) .* bleach_region_indicator));
end
...
for current_frame = 1:exp_sim_param.number_of_postbleach_frames
    rc_postbleach(current_frame) = mean(mean( C_postbleach(:, :, current_frame) .* bleach_region_indicator));
end
